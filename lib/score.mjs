...
export function authCoverage(endpoints) {
  // Conservative formula: UNKNOWN endpoints count against coverage. A route
  // we couldn't classify is not a confirmed guard. Excluding UNKNOWN from
  // the denominator silently inflates the score and contradicts the
  // "static analysis cannot prove runtime authz" framing of the report.
  // Intentionally-public endpoints (sign-up/login/refresh marked by the
  // PUBLIC_INTENTIONAL heuristic, see lib/heuristics.mjs) ARE excluded —
  // they're declared, public-by-design, and not a coverage gap.
  const counted = endpoints.filter(e =>
    !e.intentionalPublic &&
    (e.posture === "GUARDED" || e.posture === "OPEN" || e.posture === "UNKNOWN")
  );
...
export function openEndpointRisk(endpoints) {
  ...
  for (const e of endpoints) {
    ...
    if (e.intentionalPublic) continue;
